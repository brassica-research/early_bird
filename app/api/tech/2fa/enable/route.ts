import { enableTech2fa } from "@/lib/tech-auth";
...
// POST /api/tech/2fa/enable — verify a code against the pending secret and turn
// on two-factor for the signed-in technician.
export async function POST(request: Request) {
  ...
  const result = await enableTech2fa(techId, parsed.data.token);
